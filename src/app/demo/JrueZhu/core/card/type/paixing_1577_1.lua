...
-- @Last Modified time 2018-07-16 15:40:54
...
M.description = [[
功能描述说明：
	杂五十K：由一张5，一张10，一张K组成的牌型，至少有1张花色不一样
	花色指：黑、红、梅、方四种花色
]]
...
function M:findFitCards(sourceList)
	...
	local function pickCard()
		--总会凑齐五十K使得curIndex大于3，但是需要校验花色是否完全一致。
		if curIndex > 3 then
			local curColor, sameColorCount = nil, 0;
			for _, card in ipairs(pickedCards) do
				if curColor and curColor == card.color then
					sameColorCount = sameColorCount + 1;
				end
				curColor = card.color;
			end

			if sameColorCount < 2 then
