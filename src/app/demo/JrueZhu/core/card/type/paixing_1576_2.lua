--[[--ldoc desc
@module paixing_1576_2
@author LucasZhen

Date   2018-01-09 19:35:02
Last Modified by   LucasZhen
Last Modified time 2018-05-15 14:39:14
]]
...
local function _isCanInstead(targetCard,laiZiList,ruleDao)
	for i,card in ipairs(laiZiList) do 
		if not CardUtils.isTargetLaizi(ruleDao,card, targetCard) then
			return false
		end
	end
	return true
		...
		if #laiZiList ~= 0 then
			for normalByte,num in pairs(self.m_byteNum) do
				...
				if n < num then
					...
					if not _isCanInstead(targetCard,laiZiList,data.ruleDao) then 
						return false
					end				
				end 
			end 
